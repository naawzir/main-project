...
if [ ! -f $ENV_PATH ]; then
    ...
    docker-compose -f "${BASH_SOURCE%/*}/docker-compose.yml" exec -T webserver //bin/bash -c "mysql --user=\"root\" --password=\"\" --execute=\"CREATE DATABASE site_dev_tcp;GRANT ALL PRIVILEGES ON * . * TO 'root'@'127.0.0.1';FLUSH PRIVILEGES;\""
    ...
    if [ $? -eq 0 ]; then
        ...
        docker-compose -f "${BASH_SOURCE%/*}/docker-compose.yml" exec -T webserver //bin/bash -c "cd /data/srv/nginx/laravel.tcp-ltd.co.uk/site && composer install --no-progress --no-interaction --optimize-autoloader --no-ansi && php artisan key:generate"

        if [ $? -eq 0 ]; then
            docker-compose -f "${BASH_SOURCE%/*}/docker-compose.yml" exec -T webserver //bin/bash -c "cd /data/srv/nginx/laravel.tcp-ltd.co.uk/site && php artisan migrate && php artisan db:seed" # && php artisan passport:keys"
